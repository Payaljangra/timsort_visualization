...
function getValsArray(valsStr) {
    const temp = valsStr.split(' ').map(strVal => {
        return parseFloat(strVal)
    });
    return temp.filter(num => !Number.isNaN(num))
}
...
submitBtn.addEventListener('click', () => {
    ...
    if (!validate(vals, count)) {
        alert('Count and values count match failed, try again.')
        return
    }
    ////////////////restore nodes color if add node button clicked after sort/////////////////
